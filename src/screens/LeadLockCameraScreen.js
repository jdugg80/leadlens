...
export default function LeadLockCameraScreen({ navigation }) {
  ...
  const { isProcessing, startProcessing, updateMessage, stopProcessing } = useProcessing();
  ...
  const detectingAbortRef = useRef(null);   // AbortController for in-flight detection
  const pausedDuringDetectRef = useRef(false); // detection finished while paused
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Cleanup: abort any in-flight detection on unmount
  ...
  const handleTakePhoto = async () => {
    try {
      ...
      if (isPaused) {
        showToast('Camera is paused. Resume to capture.', 'error');
        return;
      }
      if (isStopped) {
        showToast('Camera is stopped. Go back to restart.', 'error');
        return;
      }
      if (!location?.zip) {
  ...
  const handleDetectBusinesses = async (base64, exifData) => {
    // Bail immediately if stopped
    if (isStopped) {
      console.log('[LeadLockCamera] Detection skipped — user stopped');
      return;
    }

    // Set up abort controller for cancellation
    const abortController = new AbortController();
    detectingAbortRef.current = abortController;
    console.log('[LeadLockCamera] Detection started');
    startProcessing('Analyzing businesses...');

    setDetecting(true);
    try {
      const result = await detectMultipleBusinessesInPhoto(base64, location);

      // Check if stopped or aborted while detection was in flight
      ...
      if (result.success) {
        ...
        // If paused, stash results but don't surface them yet
        if (isPaused) {
          console.log('[LeadLockCamera] Detection finished while paused — deferring result display');
          pausedDuringDetectRef.current = true;
          setDetectionResult({ ...result, formatted });
          setSelectedBusinesses(formatted.map(b => ({ ...b, selected: true })));
        } else {
          setDetectionResult({ ...result, formatted });
          setSelectedBusinesses(formatted.map(b => ({ ...b, selected: true })));
        }
      ...
      if (abortController.signal.aborted || error?.name === 'AbortError') {
        console.log('[LeadLockCamera] Detection aborted');
        stopProcessing();
        return;
      }
      if (isStopped) {
        console.log('[LeadLockCamera] Detection error ignored — user stopped');
        stopProcessing();
        return;
      }
  ...
  const handleAddToQueue = async () => {
    ...
    startProcessing('Adding to queue...');
    try {
      let resolved;
      if (resolvedZipRef.current) {
        resolved = {
  ...
  // Reset to camera
  const _resetCamera = () => {
    console.log('[LeadLockCamera] resetCamera called');
        ...
        <View style={[s.header, s.headerAbsolute, { paddingTop: insets.top + 12 }]}>
          ...
          {isPaused && (
            <View style={s.pausedIndicator}>
              <Text style={s.pausedIndicatorText}>⏸ PAUSED</Text>
            </View>
          )}
        </View>
