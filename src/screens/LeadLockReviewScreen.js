...
export default function LeadLockReviewScreen({ navigation, route }) {
  useEffect(() => {
    BetaTracker.screen('LeadLockReviewScreen');
  }, []);
  ...
  const analyze = async () => {
    ...
    BetaTracker.crash('LeadLockReviewScreen', resolverError);
    ...
    BetaTracker.crash('LeadLockReviewScreen', err);
  ...
  return (
    <KeyboardAvoidingView style={s.root} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <ScrollView style={s.scroll} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        {/* OCR candidates */}
        {!analyzing && !!ocrResult && (ocrResult.businessNameCandidates?.length > 0 || ocrResult.addressCandidates?.length > 0 || ocrResult.taggedResults?.length > 0) && (
          <>
            <SectionLabel>OCR Clues</SectionLabel>
            <Card>
              {ocrResult.taggedResults?.map((tag, idx) => (
                <View key={`tag-${idx}`} style={s.tagClueRow}>
                  <Text style={s.tagClueType}>{tag.source.replace('tag-', '').replace(/_/g, ' ')}:</Text>
                  <Text style={s.tagClueText}>{tag.lead.businessName || tag.visibleTextLines[0] || '---'}</Text>
                </View>
              ))}
...
const s = StyleSheet.create({
  ...
  tagClueRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 6, gap: 8 },
  tagClueType: { color: COLORS.accent, fontSize: 11, fontWeight: '800', textTransform: 'uppercase', width: 80 },
  tagClueText: { color: COLORS.text, fontSize: 13, fontWeight: '700', flex: 1 },
