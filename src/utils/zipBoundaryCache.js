...
const CACHE_KEY = '@leadlens_zip_bounds_v5';
const GOOGLE_MAPS_API_KEY = 'AIzaSyBjzIQsLGY1E3paPr8XROVWg83e_JLOJzI';

// State GeoJSON files from OpenDataDE on GitHub — real Census TIGER boundaries
const STATE_FILES = {
...
const GITHUB_BASE = 'https://raw.githubusercontent.com/OpenDataDE/State-zip-code-GeoJSON/master';
const STATE_CACHE_KEY = '@leadlens_state_zip_data';
...
const ZIP_PREFIX_STATE = {
  ...
  '200':'VA','201':'VA','220':'VA','221':'VA','222':'VA','223':'VA','224':'VA','225':'VA','226':'VA','227':'VA','228':'VA','229':'VA','230':'VA','231':'VA','232':'VA','233':'VA','234':'VA','235':'VA','236':'VA','237':'VA','238':'VA','239':'VA','240':'VA','241':'VA','242':'VA','243':'VA','244':'VA','245':'VA','246':'VA',
...
async function saveToCache(zip, entry) {
...
function getStateForZip(zip) {
  ...
  return ZIP_PREFIX_STATE[prefix] || null;
}

async function fetchStateData(stateCode) {
  if (_stateDataCache[stateCode]) return _stateDataCache[stateCode];

  const fileName = STATE_FILES[stateCode];
  if (!fileName) return null;

  try {
    const url = `${GITHUB_BASE}/${fileName}_zip_codes_geo.min.json`;
    console.log(`[GeoJSON] Fetching state data: ${stateCode} from ${url}`);
    ...
    if (!resp.ok) {
      console.log(`[GeoJSON] Failed: ${resp.status}`);
      return null;
    }

    const data = await resp.json();
    console.log(`[GeoJSON] ${stateCode} loaded: ${data.features?.length} features`);

    // Build ZIP lookup from GeoJSON
    const lookup = {};
    for (const feature of data.features || []) {
      const zipCode = feature.properties?.ZCTA5CE10 ||
                      ...
                      feature.properties?.zip;
      if (!zipCode || !feature.geometry) continue;
      ...
      if (geom.type === 'Polygon') {
        ...
        rings = geom.coordinates.flatMap(poly =>
          poly.map(ring => ring.map(([lng, lat]) => ({ latitude: lat, longitude: lng })))
        );
      }

      const outerRings = rings.filter(r => r.length >= 3);
      if (outerRings.length) {
        const center = {
          latitude: outerRings[0].reduce((s, p) => s + p.latitude, 0) / outerRings[0].length,
          longitude: outerRings[0].reduce((s, p) => s + p.longitude, 0) / outerRings[0].length,
        };
        lookup[String(zipCode).padStart(5, '0')] = { polygon: outerRings[0], allRings: outerRings, center, source: 'census-geojson' };
      }
    }

    _stateDataCache[stateCode] = lookup;
    return lookup;
  } catch (err) {
    console.log(`[GeoJSON] Error: ${err.message}`);
    return null;
  }
}

async function fetchFromGoogle(zip) {
  try {
    const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${zip}&components=postal_code:${zip}|country:US&key=${GOOGLE_MAPS_API_KEY}`;
    const controller = new AbortController();
    ...
    return { polygon, allRings: [polygon], center: loc ? { latitude: loc.lat, longitude: loc.lng } : null, source: 'google' };
  } catch { return null; }
}

export async function getZipBounds(zip) {
  const cache = await getCache();
  if (cache[zip] && cache[zip].source === 'census-geojson') return cache[zip];

  // Get state for this ZIP and fetch state GeoJSON
  const stateCode = getStateForZip(zip);
  if (stateCode) {
    const stateData = await fetchStateData(stateCode);
    if (stateData?.[zip]) {
      await saveToCache(zip, stateData[zip]);
      return stateData[zip];
    }
  }

  // Fallback to Google
  const entry = await fetchFromGoogle(zip);
  if (entry) await saveToCache(zip, entry);
  return entry;
}

export async function clearZipBoundsCache() {
  _memCache = {};
  _stateDataCache = {};
